...
// =======================================================
// 中分類・小分類を完全網羅したビッグデータ構築用キーワード（200個以上）
// =======================================================
const KEYWORDS = [
  // ファッション・アパレル
...
// 1カテゴリにつき最大30ページ（900件）取得する。
// 150カテゴリ × 900件 = 最大 135,000 件 の超ビッグデータ！
const PAGES_PER_KEYWORD = 30; 

async function runSync() {
  console.log("🚀 超ビッグデータ構築パトロールを開始します...");
  let totalSaved = 0;

  for (const keyword of KEYWORDS) {
    console.log(`\n🔍 カテゴリ: [ ${keyword} ] のデータを仕入中...`);
    ...
    for (let page = 1; page <= PAGES_PER_KEYWORD; page++) {
      ...
      // 楽天API制限（1秒に1回）を確実に守るためのウェイト
      await new Promise(r => setTimeout(r, 1500));
    }
  }
  
  console.log(`\n\n🎉 完了！ 合計 ${totalSaved} 件の商品データをSupabaseに格納しました！`);
  process.exit(0);
}
